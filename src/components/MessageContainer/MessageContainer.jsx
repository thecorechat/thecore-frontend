...
import { formatDividerDate, formatMessageTime, getDateKey } from '../../../lib/utils';
            ...
            {grouped[dateKey].map((message) => (
              ...
              /> */}
                ...
                <div style={{ position: 'relative', marginBottom: '15px' }}>
                  <ChatHeader>
                    <ChatName onClick={onOpenUserProfile}>{message.name === owner ? 'You' : message.name}</ChatName>
                    <ChatTime>{formatMessageTime(message.createdAt)}</ChatTime>
                  </ChatHeader>

                  <ChatBubble isOwner={message.name === owner}>
                    {message.image && <ChatImageAttachment src={message.image} alt="Attachment" />}
                  ...
                  >
                    <Like>
                      {message.isLiked ? <FaHeart size={14} /> : <FaRegHeart size={14} />}
